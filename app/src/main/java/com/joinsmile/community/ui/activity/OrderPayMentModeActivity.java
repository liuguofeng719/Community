...
public class OrderPayMentModeActivity extends BaseActivity {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SDK_PAY_FLAG:
                    ...
                    break;
                case SDK_CHECK_FLAG:
                    Toast.makeText(OrderPayMentModeActivity.this, "检查结果为：" + msg.obj, Toast.LENGTH_SHORT).show();
                    break;
                default:
                    break;
            }
    ...
    private void wenXinSubmit() {
        mDialog = CommonUtils.showDialog(OrderPayMentModeActivity.this, "正在加载微信支付");
        mDialog.show();
        api = WXAPIFactory.createWXAPI(OrderPayMentModeActivity.this, Constants.wxpay.APPID);
        api.registerApp(Constants.wxpay.APPID);

        Call<WXPayVo> callWX = getApisNew().payProductOrderByWeiChat(extras.getString("orderId")).clone();
            ...
            public void onResponse(Call<WXPayVo> call, Response<WXPayVo> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (response.body() != null) {
                        ...
                        CommonUtils.make(OrderPayMentModeActivity.this, body.getErrorMessage() == null ? response.message() : body.getErrorMessage());
                    } else {
                        CommonUtils.make(OrderPayMentModeActivity.this, CommonUtils.getCodeToStr(response.code()));
                    }
                }
            }
    ...
    private void alipaySubmit() {
        Runnable payRunnable = new Runnable() {
            ...
            public void run() {
                // 构造PayTask 对象
                PayTask alipay = new PayTask(OrderPayMentModeActivity.this);
                // 调用支付接口，获取支付结果
    ...
    private void signAlipay() {
        Call<AlipayVo> siginCall = getApisNew().payProductOrderByAlipay(extras.getString("orderId")).clone();
